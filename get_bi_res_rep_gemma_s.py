...
layer_num_end = 16
file_name = 'dolly'
res_logits_str = f'gemma_3_12b_{layer_num_start}2{layer_num_end}_{file_name}_avg100'
# res_logits_str = f'gemma_3_12b_bi_res_24_{file_name}_avg100'
model_id = '/mnt/petrelfs/share_data/safety_verifier/models/gemma-3-12b-it'
dir_id = 'gemma_3_12b_sort_results'
...
with open(f'/mnt/petrelfs/share_data/lihao1/data/train_data/dolly.json','r') as f:
    data = []
...
with open('data/pure-bad-100-gemma-3-12b-anchor1.jsonl', 'r') as f:
    safe_data = []
    for line in f:
...
def alpaca_data_process(data, is_alpaca = True):
    ...
    with torch.inference_mode():
        ...
        # rep = model(input_ids).hidden_states[24][0][-1].detach().cpu()
