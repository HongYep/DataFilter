...
res_logits_str = f'llama_bi_res_{layer_num}_{file_name}_avg100'
...
with open(f'data/train_data/{file_name}.json','r') as f:
    data = []
...
def alpaca_data_process(data, is_alpaca = True):
    ...
    with torch.inference_mode():
        ...
        rep = model(input_ids).hidden_states[layer_num][0][-1].detach().cpu()
    return rep
