...
def data_preprocess(data, data_type = 'alpaca'):
    if data_type == 'alpaca':
        ...
        example = torch.tensor(prompt.tolist() + tokenizer(f"{data['output']}</s>")['input_ids'][1:])
...
if __name__ == '__main__':
    ...
    # calculate_mean(safe_data_bench, 'mistral_safe_hate_speech_anchor1', max_response_length=10)
    with open('data/alpaca-gpt4-clean.json','r') as f:
        safe_data = json.load(f)
    safe_tensor = torch.load('mistral_safe_hate_speech_anchor1/grads-mean.pt', weights_only=True).to('cuda:6')
    unsafe_tensor = torch.load('mistral_unsafe_hate_speech/grads-mean.pt', weights_only=True).to('cuda:7')
    ...
    with open(f'mistral_sort_results/grads/clean-grads.json','w') as f:
