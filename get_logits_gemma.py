...
logits_str = 'clean-logits_res' if res_logits else 'clean-logits'
...
# with open('data/Med/GenMedGPT-sample_clean.json', 'r') as f:
    data = []
    # for line in f:
...
def alpaca_data_process(data, is_alpaca = True):
    ...
    with torch.inference_mode():
        ...
        # res_input_ids = tokenizer.apply_chat_template(
        #     message,
        #     add_generation_prompt=res_logits,
        #     return_tensors="pt"
        # ).to(model.device)
...
with open(f"{dir_id}/{logits_str}_top1.json",'w') as file:
...
with open(f"{dir_id}/{logits_str}_top1_top_{select_n}.json",'w') as file:
...
with open(f"{dir_id}/{logits_str}_top1_bottom_{select_n}.json",'w') as file:
