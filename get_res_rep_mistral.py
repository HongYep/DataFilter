...
model_id = '/mnt/petrelfs/lihao1/trustai/share/models/mistralai/Mistral-7B-Instruct-v0.3'
...
def alpaca_data_process(data, is_alpaca = True):
    ...
    prompt = torch.tensor(tokenizer.apply_chat_template(message, add_generation_prompt=True))
    res_ids = torch.tensor(prompt.tolist() + tokenizer(f"{data['output']}</s>")['input_ids'][1:]).to(model.device)
    res_ids = torch.unsqueeze(res_ids,0)
    with torch.inference_mode():
        rep = model(res_ids).hidden_states[-1][0][-1].cpu()
    return rep
...
def top_cosine_similarity(A, B, avg_n = 10):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--res_str', type=str, default='OncoGPT')
    # data/Med/OncoGPT-sample_clean.json
    parser.add_argument('--input_json', type=str, default='data/Med/OncoGPT-sample_clean.json')
    args = parser.parse_args()
    res_rep_str = args.res_str + "_res"
